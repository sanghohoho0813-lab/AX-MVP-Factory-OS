...
## 컨설팅 작업실 — P2 (2026-09-08, 브랜치 `claude/patent-venture-mvp-studio-v1`)
- 증빙 슬롯에 파일 업로드(지금은 주장·출처 메타만). 기존 `client-documents` 버킷 하위 `{ws}/{client}/consulting/{project}/…` 경로 재사용 권장.
- 전역 KIPO 참고자료 표(`consulting_references`) — 지금은 118종을 코드에 두고 선택만 저장. 워크스페이스가 직접 추가할 사례가 생기면 표로.
- 오늘 화면의 '컨설팅 다음 행동' 을 Top 3 점수에 합산할지 — 지금은 별도 구역. 실제로 매일 보게 되면 합친다.
- 결과 들여오기에서 .docx/.pdf 텍스트 추출 — 지금은 .md/.txt 만.
- 사실표 스냅샷을 S10 완료 시 자동으로 산출물(`VENTURE_FACTSHEET_SNAPSHOT`)로 남기기 — 지금은 사람이 '직접 적기' 로 저장.

## 간단 모드 — P2 (2026-09-08, 브랜치 `claude/simple-operator-ux-v1`)
- **간단 모드 안에서 되돌리기** — 잘못 고른 선택지를 지금은 [고급] 으로 가서 고쳐야 한다. 카드에 "방금 고른 것 바꾸기" 한 줄이면 충분하다.
- **프롬프트 패널 복원** — 새로고침하면 방금 만든 프롬프트 패널이 사라진다(할 일 자체는 '결과 가져오기' 로 남는다). 마지막 프롬프트를 결과물 탭이 아니라 진행하기 위에 다시 띄우는 편이 낫다.
- **산출물 버전 비교** — 결과물 탭은 종류별 최신 하나만 보여 준다. v1↔v2 를 나란히 보는 화면은 실제로 두 번 이상 돌려 본 뒤에 만든다.
- **간단 모드 supabase 검증** — 표는 적용됐으므로 Preview 에서 프로젝트 하나를 끝까지 밀어 저장·재조회를 사람이 한 번 확인한다.
- **첫 사용자 관찰** — §47 의 5개 인수 질문은 E2E 로 대신했다. 대표가 설명 없이 처음 여는 장면을 한 번 보고 그때 고친다.
- **올린 서류 보관** — 지금은 글자만 읽고 파일은 버린다. `client-documents` 버킷의 `{ws}/{client}/consulting/{project}/` 아래에 함께 넣어 두면 나중에 근거 서류를 다시 찾을 필요가 없다. 실제로 두 번 이상 다시 찾게 되면 만든다.
- **읽은 값 되돌리기** — 서류로 덮어쓴 뒤 "이전 값으로" 한 번에 돌리는 버튼. 지금은 고급 사실표에서 고쳐야 한다.

## 마찰 제거 — P2 (2026-09-08, 브랜치 `claude/zero-friction-ux-v2`)
- **되돌리기(Undo)** — 방금 저장한 값을 한 번에 되돌리는 버튼. 지금은 고급 사실표에서 고쳐야 한다. 채점에서 가장 낮은 축(93)이다.
- **미룬 항목을 '오늘 할 일' 에 올리기** — 지금은 프로젝트 안에서만 보인다. 여러 프로젝트의 '확인 필요' 를 오늘 화면에 모으면 한 번에 챙길 수 있다.
- **실제 초보 관찰** — 55세 컨설턴트가 아무 설명 없이 여는 장면을 한 번 보고, 멈추는 자리만 고친다. E2E 로는 여기까지가 한계다.
- **초안 문장 다듬기** — 지금은 문장 틀 5개. 실제로 몇 번 써 본 뒤 자주 고치게 되는 자리만 틀을 손본다.
- **참고자료 추천 근거 강화** — 지금은 낱말 겹침이다. 실제로 고른 이력이 쌓이면 그것을 우선순위에 반영할지 정한다.
