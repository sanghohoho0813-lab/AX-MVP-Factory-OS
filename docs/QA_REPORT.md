...
Production 미배포 · main 미병합 · 마이그레이션 0012 미적용 (D-44). 아래는 local 모드 실측이다.
...
- Vercel Preview URL — 이 세션에는 Vercel 조회 도구가 없어 주소를 받지 못했다. 푸시 뒤 Vercel 대시보드의 이 브랜치 Preview 로 확인한다.
