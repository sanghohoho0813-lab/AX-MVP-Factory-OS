...
/* ---- 1440 ---- */
{
  ...
  // D-122: 계약 완료는 확인 시트 — 받을 날 · 영업자 · 계약 방식까지 한 번에
  const sheet = page.getByTestId('contract-close')
  await sheet.waitFor()
  check('계약 완료: 확인 시트가 뜬다(바로 옮기지 않음)', (await page.evaluate((id) => JSON.parse(localStorage.getItem('axmvp.v1.operations_clients')).find((c) => c.id === id).sales.stage, stored.id)) !== 'contracted')
  ...
  check('계약 완료: 수금 항목이 받을 날(계약 7일 뒤)과 함께 생긴다', s3.fees.length >= 1 && s3.fees.every((f) => /^\d{4}-\d{2}-\d{2}$/.test(f.dueDate)), JSON.stringify(s3.fees.map((f) => [f.label, f.amount, f.dueDate])))
...
/* ---- D-114 2단계: 미팅 준비 (1440) ---- */
{
  ...
  // D-123: 점수는 화면에 없다 — 중요도 5단계 색(숫자는 data-score 에만)
  const lvl = page.getByTestId('lead-score')
  const score1 = Number((await lvl.getAttribute('data-score')) ?? '0')
  ...
  check('미팅 준비: 회사 사정 칸은 미팅 기록 아래', !!recBox && !!profBox && profBox.y > recBox.y)
  await page.getByRole('button', { name: /미팅에서 알게 된 회사 사정/ }).click()
  await page.getByRole('button', { name: '가지급금 있음' }).click()
  await page.getByTestId('profile-save').click()
  await page.waitForTimeout(500)
  const score2 = Number((await page.getByTestId('lead-score').getAttribute('data-score')) ?? '0')
  check('미팅 준비: 체크를 켜고 저장하면 중요도 계산이 오른다', score2 > score1, `${score1} → ${score2}`)
  // 미팅 기록 — 1차
...
/* ---- D-114 3단계: 상품·제안 (1440) ---- */
{
  ...
  const closeSheet = page.getByTestId('contract-close')
  await closeSheet.waitFor()
  check('계약 완료 시트: 제안 상품 4줄 · 합계 1,050만원 · 월납 보험료 칸(보험 포함)', (await closeSheet.getByLabel(/수금 항목 \d+ 이름/).count()) === 4 && ((await closeSheet.innerText()) ?? '').includes('10,500,000') && (await closeSheet.getByLabel('월납 보험료').inputValue()) === '3,000,000')
  await page.getByTestId('contract-close-save').click()
  ...
  check('계약 완료: 계약 정보도 — 방식 · 현금 금액 · 월납 보험 한 줄 · 받을 날', saved.contract.kind !== '' && saved.contract.cashAmount === 10_500_000 && saved.contract.policies.some((p) => p.monthlyPremium === 3_000_000) && saved.fees.every((f) => f.dueDate !== ''), JSON.stringify(saved.contract))
