...
const DEAL_STAGES = [
  ...
  { key: "contacted", label: "첫 연락", icon: "📞", color: C.sky, bg: "#E0F2FE" },
  ...
  { key: "docs_received", label: "자료수령", icon: "📥", color: C.sky, bg: "#E0F2FE" },
...
function Card({ children, style, onClick }) {
  return <div className="ui-card ui-hover" onClick={onClick} style={{ background: C.card, border: "1px solid " + C.bdr, borderRadius: 16, ...style }}>{children}</div>;
}
...
const PROPOSAL_STATE_STYLE = { "제안 전": [C.textM, "#EEF2F7"], "제안서 작성": [C.blue, C.blueBg], "제안 완료": [C.sky, "#E0F2FE"], "견적 전달": [C.purple, C.purpleBg], "검토 중": [C.warn, C.warnBg], "조건 조율": [C.warn, C.warnBg], "계약 예정": [C.gold, "#FCEFDA"], "계약 완료": [C.ok, C.greenBg], "보류": [C.textM, "#EEF2F7"] };
...
function sourceTag(rec) {
  ...
  if (rec && (rec.source === "manual" || rec.sample === false)) return { label: "직접 등록", color: C.sky, bg: "#E0F2FE" };
...
const DB_SOURCE_STYLE = { "기존 고객 소개": [C.ok, C.greenBg], "지인 소개": [C.ok, C.greenBg], "온라인 광고 DB": [C.purple, C.purpleBg], "회사 제공 DB": [C.blue, C.blueBg], "사업단 제공 DB": [C.blue, C.blueBg], "직접 발굴": [C.sky, "#E0F2FE"] };
...
function CustomerPanel({ item, data, setData, goMeeting, onConvert, inDB }) {
  ...
  return <div style={{ display: "grid", gap: 12 }}>
    ...
    <Card style={{ padding: 18 }}><Label>예상 니즈</Label><div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 6 }}>{(item.interests || []).length ? item.interests.map((x) => <Badge key={x} color={C.sky} bg="#E0F2FE">{x}</Badge>) : <span style={{ color: C.textM, fontSize: "calc(var(--s,1.3)*15px)" }}>상담 과정에서 확인 필요</span>}</div></Card>
    </AccordionSection>
            ...
            <div style={cardBox}>
              ...
              {dbg ? <>
                ...
                <div style={{ border: `1px solid ${C.bdr}`, borderRadius: 8, maxHeight: 220, overflow: "auto", background: "#fff", fontFamily: "ui-monospace, Menlo, monospace", fontSize: "calc(var(--s,1.3)*11px)" }}>
                  {dl.length === 0 ? <div style={{ padding: "8px", color: C.textM }}>표시할 라인이 없습니다{lineQ ? ` (검색: ‘${lineQ}’)` : ""}.</div> : dl.slice(0, 400).map((l) => <div key={l.li} onClick={() => setSelLi(l.li)} style={{ display: "flex", gap: 6, padding: "2px 8px", cursor: "pointer", background: selLi === l.li ? "#FFF3D6" : (l.hasAcct ? "#EFF6FF" : "transparent"), borderBottom: `1px solid ${C.bg}` }}><span style={{ color: C.textM, minWidth: 34, textAlign: "right", flexShrink: 0 }}>L{l.li}</span><span style={{ color: l.hasAcct ? C.blue : C.text, wordBreak: "break-all" }}>{lineQ ? cretopHl(l.text, lineQ.trim()) : l.text}</span></div>)}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.1fr .9fr", gap: 14 }}>
        ...
        <Card style={{ overflow: "hidden" }}>
          ...
          <div style={{ padding: 16 }}>
            <Badge color={C.sky} bg="#E0F2FE">{contentSeed.cat}</Badge>
...
function ContentCard({ item, data, setData }) {
  ...
  return <Card style={{ padding: 14 }}>
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}><Badge color={C.sky} bg="#E0F2FE">{cat}</Badge>{n > 0 && <Badge color={C.gold} bg="#FCEFDA">관련 고객 {n}</Badge>}</div>
...
function EduCard({ item, data, setData }) {
  ...
  return <Card style={{ padding: 14 }}>
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}><Badge color={C.sky} bg="#E0F2FE">{cat}</Badge>{e.level && <Badge color={C.purple} bg={C.purpleBg}>{e.level}</Badge>}{e.audience && <Badge color={C.textM} bg="#EEF2F7">{e.audience}</Badge>}{n > 0 && <Badge color={C.gold} bg="#FCEFDA">관련 고객 {n}</Badge>}</div>
...
function StratCard({ item, data, setData }) {
  ...
  return <Card style={{ padding: 14 }}>
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}><Badge color={C.sky} bg="#E0F2FE">{s.cat}</Badge><Badge color={C.warn} bg={C.warnBg}>세무사 검토 권장</Badge>{rel.length > 0 && <Badge color={C.gold} bg="#FCEFDA">관련 고객 {rel.length}</Badge>}</div>
...
function CustomerReports({ data, setData, goMeeting }) {
  ...
  return <div>
    ...
    {all.length === 0 ? <Card style={{ padding: 40, textAlign: "center" }}><p style={{ color: C.textM, fontSize: "calc(var(--s,1.3)*16px)", lineHeight: 1.7, marginBottom: 14 }}>아직 고객이 없습니다. 고객을 등록하면 그 고객을 골라 방문용 리포트·제안서·견적서를 만들 수 있습니다.</p><SampleCTA data={data} setData={setData} label="🎁 샘플로 먼저 보기" /></Card> : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(340px,1fr))", gap: 14 }}>{all.map((c) => { const bd = scoreBand(scoreLead(c)); const src = sourceTag(c); return <Card key={c.id} style={{ padding: 18 }}>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", margin: "10px 0" }}>{(c.interests || []).slice(0, 4).map((x) => <Badge key={x} color={C.sky} bg="#E0F2FE">{x}</Badge>)}</div>
  ...
  const cardOf = (pp) => <Card key={pp.id} style={{ padding: "12px 14px" }}>
      <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        <div style={{ flex: "1 1 240px", minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}><b style={{ fontSize: "calc(var(--s,1.3)*16px)" }}>{pp.name}</b><Badge color={C.sky} bg="#E0F2FE">{pp.cat}</Badge></div>
  ...
  return <div>
    ...
    <Card style={{ padding: 20, marginBottom: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 12 }}>{perf.map((p) => <div key={p.name} style={{ background: C.bg, borderRadius: 11, padding: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "start" }}><b style={{ fontSize: "calc(var(--s,1.3)*15px)" }}>{p.name}</b><Badge color={C.sky} bg="#E0F2FE">{p.cat}</Badge></div>
...
function Pipeline({ data, setData, goMeeting }) {
  ...
  const card = (c) => {
    ...
    return <div key={c.id} style={{ background: C.bg, border: "1px solid " + C.bdr, borderRadius: 11, padding: 12, marginBottom: 8 }}>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", margin: "6px 0" }}>{(c.interests || []).slice(0, 3).map((x) => <Badge key={x} color={C.sky} bg="#E0F2FE">{x}</Badge>)}</div>
...
function Settings({ data, setData, reset, pinSet, applyLock, removePin, onLockNow }) {
  ...
  return <div style={{ display: "grid", gap: 14 }}>
    ...
    <Card style={{ padding: 20 }}><h3 style={h3}>⑥ 저장 방식 안내</h3><div style={{ fontSize: "calc(var(--s,1.3)*16px)", color: C.textS, lineHeight: 1.8 }}><div>• [이 OS] 영업 기록은 <b>미래에이아이랩 OS 의 모듈 기록</b>에 저장됩니다. 클라우드 모드면 작업실 안 다른 기기에서도 보입니다.</div><div>• 업체(업체명·대표자)는 <b>고객 운영</b> 업체를 그대로 씁니다. 여기서 지워도 고객 운영 업체는 남습니다.</div></div>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
...
export default function App({ tab: tabProp = "briefing", onTab, focus }) {
  ...
  return <div className={"appRoot" + (drawerOpen ? " drawer-open" : "")} data-scale={scaleName} data-testid="sales-orig" style={{ background: C.bg, color: C.text, fontFamily: FF, fontSize: "calc(var(--s) * 18px)", borderRadius: 16, overflow: "hidden" }}>
    <style>{`
      ...
      .ui-card{transition:transform .18s cubic-bezier(.22,.61,.36,1),box-shadow .18s ease,border-color .18s ease;box-shadow:0 1px 2px rgba(15,23,42,.04),0 1px 3px rgba(15,23,42,.06)}
