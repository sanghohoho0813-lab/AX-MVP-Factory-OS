          ...
          {evidence.map((e) => (
            <tr key={e.id} className="border-t border-slate-100 align-top">
              ...
              <td className="px-3 py-2">
                <p className="text-[0.875rem] font-medium text-slate-700">{e.questionCode}</p>
                <p className="max-w-[220px] truncate text-[0.875rem] text-slate-400">{e.questionText}</p>
              </td>
              <td className="px-3 py-2 text-[0.875rem] break-keep text-slate-700">
                {e.answerValue}
              </td>
    ...
    <div className="rounded-(--radius-card) border border-slate-200">
      ...
      {open && (
        <div className="border-t border-slate-100 px-4 py-3">
          <p className="mb-2 text-[0.875rem] break-keep text-slate-600">{score.explanation}</p>
          {adjustment && (
...
export function DeductionList({
  ...
  if (deductions.length === 0) {
    return <p className="text-[0.875rem] text-slate-500">적용된 감점이 없습니다.</p>
  }
  return (
      ...
      {deductions.map((d) => (
        ...
        >
          <div className="flex flex-wrap items-center gap-2">
            ...
            {d.overridden && (
              <span className="rounded-md border border-slate-200 bg-white px-1.5 py-0.5 text-[0.875rem] font-medium text-slate-500">
                제외됨
              </span>
            )}
            {editable && onToggleOverride && (
              <button
                ...
                className="ml-auto inline-flex cursor-pointer items-center gap-1 text-[0.875rem] font-medium text-brand-600 hover:text-brand-700"
              >
                {d.overridden ? (
                  <>
