          ...
          {evidence.map((e) => (
            <tr key={e.id} className="border-t border-slate-100 align-top">
              ...
              <td className="px-3 py-2">
                ...
                <p className="max-w-[220px] truncate text-xs text-slate-400">{e.questionText}</p>
              </td>
    ...
    <div className="rounded-(--radius-card) border border-slate-200">
        ...
        <div className="flex items-center gap-3 sm:w-64">
          ...
          <span className="w-20 shrink-0 text-right text-sm font-semibold text-slate-800">
            {score.measured ? (
              <>
                {score.rawScore}
                <span className="text-xs font-normal text-slate-400"> / {score.maxScore}</span>
              </>
            ) : (
              <span className="text-xs font-normal text-slate-400">데이터 없음</span>
            )}
          </span>
        </div>
      ...
      {open && (
        <div className="border-t border-slate-100 px-4 py-3">
          ...
          {adjustment && (
            <p className="mb-2 rounded-(--radius-control) border border-accent-200 bg-accent-50 px-3 py-2 text-xs break-keep text-accent-700">
          ...
          {score.warnings.map((w) => (
            <p
              key={w}
              className="mb-2 flex items-start gap-1.5 text-xs break-keep text-warning-700"
            >
...
export function ManualAdjustNotice({ show }: AdjustNoticeProps) {
  if (!show) return null
  return (
    <p className="rounded-(--radius-control) border border-accent-200 bg-accent-50 px-3 py-2 text-xs break-keep text-accent-700">
