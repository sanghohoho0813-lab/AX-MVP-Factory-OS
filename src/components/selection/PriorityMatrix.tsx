  ...
  return (
    <div className="mx-auto w-full max-w-2xl">
      <div className="relative aspect-square w-full rounded-(--radius-panel) border border-slate-200 bg-white">
        ...
        {/* 사분면 라벨 */}
        <span className="absolute top-2 left-2 text-[0.875rem] font-medium text-accent-600">전략적 투자</span>
        <span className="absolute top-2 right-2 text-[0.875rem] font-medium text-success-700">빠른 실행</span>
        <span className="absolute bottom-2 left-2 text-[0.875rem] font-medium text-slate-400">후순위 보류</span>
        <span className="absolute right-2 bottom-2 text-[0.875rem] font-medium text-warning-700">선행 준비 필요</span>
        ...
        {points.map(({ candidate, x, y }) => (
          ...
          >
            ...
            />
            <span className="pointer-events-none absolute bottom-full left-1/2 mb-1 hidden -translate-x-1/2 rounded-md bg-navy-900 px-2 py-1 text-[0.875rem] whitespace-nowrap text-white group-hover:block group-focus:block">
              {candidate.name}
            </span>
          </button>
        ))}
              ...
              {items
                ...
                .map((c) => (
                  <li key={c.id}>
                    ...
                    >
                      <span className="min-w-0 truncate text-[0.875rem] font-medium text-slate-700">{c.name}</span>
