...
export const TUTORIAL_CHAPTERS: TutorialChapter[] = [
  {
    ...
    detailedPurpose:
      'MIRAE AI LAB OS 는 대표가 매일 쓰는 운영 도구입니다. "오늘" 화면이 오늘 무엇부터 할지 정해 주고, "고객 관리"이 업체별 업무·서류·수금·자금을 한 표로 보여 주며, "잠재고객 상담신청"에는 miraeailab.com 에서 고객이 한 일(진단·주문·서류·요청)이 들어옵니다. "업무 일기"에는 통화·결정·후속조치를 남깁니다. 진단·설계·결과자료 같은 프로젝트당 한 번 쓰는 전문 기능은 AX STUDIO 에 접혀 있습니다.',
    whyNeeded:
      '전체 그림을 먼저 이해하면 지금 화면이 왜 필요한지, 다음에 무엇을 해야 하는지 헷갈리지 않습니다.',
    prerequisites: ['별도 준비물 없음 — 처음이라면 여기서 시작하세요.'],
    tasks: [
      '"오늘" 화면에서 지금 이것부터(Top 3)·빠른 기록·상담신청를 살펴봅니다.',
      '"고객 관리"에서 업체별 현황표와 업체 상세(개요·업무·서류·수금·자금·고객 플랫폼·업무 일기·파일)를 확인합니다.',
      '"잠재고객 상담신청"과 "업무 일기"가 어디 있는지, "일정"과 "자금·지원사업"이 무엇을 보여 주는지 확인합니다.',
    ...
    nextStepHint: '다음은 "고객사·프로젝트 준비"입니다. 매일 운영만 한다면 "고객 관리"에서 첫 업체를 등록해도 됩니다.',
    estimatedMinutes: 3,
    commonMistakes: [
...
export const SCREEN_HELP: ScreenHelp[] = [
  {
    ...
    checkFirst: ['"지금 이것부터" 3개 — 규칙(마감 지남 → 결제 주문 → 지난 후속조치 → 고객 서류·요청 → 임박)으로 고른 순서입니다.', '새 상담신청가 있으면 먼저 고객사에 연결합니다.'],
    howToInput: ['"무슨 일이 있었나요?"에 통화·결정·후속조치를 바로 적습니다(Ctrl+Enter).', '하루가 끝나면 "오늘 정리하기"로 처리·남음·넘김·결정·이슈를 정리합니다.'],
    completionCriteria: ['Top 3 가 비고 새 상담신청이 0 이면 오늘 급한 일은 끝난 것입니다.'],
    nextStep: '이번 주 마감과 고객 대기 건을 "고객 관리"에서 미리 챙깁니다.',
    relatedChapterId: 'system',
  },
  {
    key: 'inbox',
    screenTitle: '잠재고객 상담신청',
...
export const SCREEN_TOURS: ScreenTour[] = [
  {
    ...
    steps: [
      ...
      { target: '[data-tour="home-top3"]', title: '지금 이것부터', body: '경고·상담신청·후속조치·자금 마감을 규칙으로 합쳐 최대 3개만 보여 줍니다. 각 항목 아래 "이유"가 순서의 근거입니다.' },
      { target: '[data-tour="home-capture"]', title: '무슨 일이 있었나요?', body: '통화·결정·후속조치를 바로 적습니다. 후속조치에 기한을 넣으면 그날 Top 3 에 올라옵니다.' },
      { target: '[data-tour="home-events"]', title: '상담신청', body: 'miraeailab.com 에서 고객이 한 일이 들어옵니다. 고객사에 연결하고 처리 상태를 남기세요.' },
  ...
  },
  {
    ...
    screenTitle: '잠재고객 상담신청',
    steps: [
      { target: null, title: '잠재고객 상담신청', body: '진단 완료·주문·서류 업로드·요청이 우선순위(지금 / 오늘 중 / 참고)와 함께 쌓입니다. 연결 → 처리 중 → 처리 완료 순서로 남깁니다.' },
    ],
  },
  {
    key: 'journal',
