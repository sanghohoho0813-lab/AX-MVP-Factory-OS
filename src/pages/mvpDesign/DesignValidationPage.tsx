                ...
                <ul className="flex flex-col gap-2">
                  {[...errors, ...others].map((c, i) => (
                    <li key={i} className="flex items-start gap-2 rounded-(--radius-card) border border-slate-200 px-3.5 py-2.5">
                      ...
                      <span className="min-w-0 text-[13px] break-keep text-slate-600">{c.message}</span>
                    </li>
                  ))}
                </ul>
              )}
              {errors.length > 0 && (
                <p className="mt-3 text-[13px] font-medium text-danger-600">오류 {errors.length}건을 해결해야 설계를 확정할 수 있습니다.</p>
              )}
            </Panel>
                ...
                {design.guardrailChecks.map((g) => (
                  <li key={g.key} className="flex items-center justify-between gap-2 rounded-(--radius-card) border border-slate-200 px-3.5 py-2">
                    <div className="min-w-0">
                      <p className="truncate text-[13px] text-slate-700">{g.label}</p>
              ...
              {design.acceptanceCriteria.length === 0 ? (
                ...
                <ul className="flex flex-col gap-2">
                  {design.acceptanceCriteria.map((a) => (
                    <li key={a.id} className="rounded-(--radius-card) border border-slate-200 px-3.5 py-2.5 text-[13px]">
                  ...
                  {design.testScenarios.map((t) => (
                    <li key={t.id} className="rounded-(--radius-card) border border-slate-200 px-3.5 py-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-[13px] font-medium text-slate-700">{t.title}</p>
                        <span className={`rounded-md border px-1.5 py-0.5 text-[11px] font-medium ${TONE_BADGE_CLASS[TEST_KIND_META[t.kind].tone]}`}>
                    ...
                    {design.kpis.map((k) => (
                      <li key={k.id} className="rounded-(--radius-card) border border-slate-200 px-3.5 py-2.5">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-[13px] font-medium text-slate-700">{k.name}</p>
                        </div>
