...
function WorkflowColumn({ workflow, tone }: { workflow: MvpWorkflow; tone: 'current' | 'desired' }) {
  return (
    <Panel title={tone === 'current' ? '현재 업무 흐름' : 'MVP 적용 후 목표 흐름'}>
      <p className="mb-3 text-[13px] break-keep text-slate-500">{workflow.summary}</p>
      {workflow.steps.length === 0 ? (
        <p className="text-[13px] text-slate-400">기록된 단계가 없습니다.</p>
      ) : (
          ...
          {workflow.steps.map((step, i) => {
            ...
            return (
              <li key={step.id} className="flex items-start gap-3 rounded-(--radius-card) border border-slate-200 px-3.5 py-2.5">
                <span className="mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full bg-slate-100 text-[11px] font-semibold text-slate-500">
                  {i + 1}
                </span>
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] break-keep text-slate-700">{step.action}</p>
