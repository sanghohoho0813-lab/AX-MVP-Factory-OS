...
function Field({ label, value }: { label: string; value: string }) {
  return <div><dt className="text-[0.8125rem] font-semibold text-slate-400">{label}</dt><dd className="mt-1 text-[0.9rem] break-keep text-slate-700">{value || '-'}</dd></div>
}

function MoneyInput({ label, value, onChange }: { label: string; value: number | null; onChange: (value: number | null) => void }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-[0.875rem] font-semibold text-slate-600">{label}</span>
    ...
    <div className="flex flex-col gap-5">
      ...
      <section className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <ul className="mt-4 grid grid-cols-1 gap-2 md:grid-cols-2">
              {CLIENT_DOCUMENTS.map((document) => {
                ...
                return <li key={document.key} className={`flex min-h-20 items-center gap-3 rounded-(--radius-card) border px-3.5 py-3 ${state.received ? 'border-success-200 bg-success-50/40' : 'border-slate-200 bg-white'}`}>
                  ...
                  <div className="min-w-0 flex-1"><p className="truncate text-[0.9rem] font-semibold text-slate-800">{document.label}</p><p className="mt-0.5 truncate text-[0.8125rem] text-slate-500">{state.fileName ?? (document.requiredForFunding ? '정책자금 공통 증빙' : '수령 확인 필요')}</p></div>
        ...
        <aside className="flex flex-col gap-5">
          ...
          ].map((item) => <div key={item.label}><div className="flex justify-between text-[0.875rem] font-semibold text-slate-500"><span>{item.label}</span><span>{item.value}%</span></div><div className="mt-1.5"><ProgressBar value={item.value} tone={item.tone} label={item.label} /></div></div>)}</div></Panel>
        </aside>
      </section>
    </div>
  )
